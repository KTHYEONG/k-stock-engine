...
class NetAlphaModel(Protocol):
    """Deterministic continuous net-alpha model contract."""
...
class ElasticNetNetAlpha:
    ...
    Features are standardized per fold using training-fold statistics only;
    the fold-local standard transform is applied to validation at predict time
    with the frozen statistics, so no validation information ever leaks.
    """
...
class LightGbmNetAlpha:
    ...
    ``objective=regression_l1`` predicts continuous net residual directly. Seed
    and thread count are pinned; ``deterministic=True`` and
    ``force_col_wise=True`` keep the booster reproducible across runs.
    """
    ...
    def manifest(self) -> ModelManifest:
        ...
        return ModelManifest(
            ...
            eligible_from=self._manifest.eligible_from,
            eligible_to=self._manifest.eligible_to,
            model_type="lightgbm_l1_net_alpha",
            params=params,
        )
...
class CalibrationState:
    ...
    ``boundaries`` are the score quantile cut points fit on OOF predictions
    only; ``buckets`` hold the per-bucket expected alpha and lower bound keyed
    by the physical category index of :func:`pl.cut` on those boundaries.
    """
...
class NetAlphaCalibrator:
    ...
    def fit(self, oof: pl.DataFrame) -> CalibrationState:
        ...
        boundaries = tuple(
            float(v) for v in np.quantile(score_values, quantiles)
        )
...
def _apply_calibration(
    ...
    Bucket assignment is a vectorized percentile cut of the score column; the
    frozen bucket statistics are joined on, so no per-row Python loop is used.
    """
