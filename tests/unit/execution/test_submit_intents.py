...
def test_plan_order_request_rejects_non_lot_multiple() -> None:
...
def test_submit_intents_sells_full_position_from_broker_snapshot() -> None:
    """Positions come from the broker snapshot."""
    ...
    context = ExecutionContext(
        ...
        broker=broker,
        state_store=store,
        price_provider=ConstantPriceProvider(100.0),
        positions=paper_positions(10),
    )
    intent = make_intent("exit-all", target_value=0.0)

    records = submit_intents([intent], context)

    assert len(records) == 1
    assert records[0].side is OrderSide.SELL
    assert records[0].submitted_quantity == 10
    assert store.get(f"order:{intent.intent_id}") is not None


def test_submit_intents_submits_sells_before_buys() -> None:
    """Sells before buys."""
    from src.execution.domain.orders import OrderSide
    ...
    sell = make_intent("sell-first", target_value=0.0, target_quantity=0)
    buy = make_intent(
        "buy-second",
        intent_id="intent-buy-second",
        idempotency_key="key-buy-second",
        instrument_id="KRX:000660",
        target_value=1_000_000.0,
    )

    records = submit_intents([buy, sell], context)

    assert [record.side for record in records] == [OrderSide.SELL, OrderSide.BUY]
