...
def test_backtest_session_builder_allows_suspended_zero_volume_bar(tmp_path) -> None:
    ...
    sessions = build_backtest_sessions(snapshot_repository=repository, calendar=SessionCalendar((day, next_day)), start=day, end=day, decision_time_of=lambda session: session)
    assert len(sessions) == 1
    # Bar for suspended day is present for mark-to-market; adtv_20d will be 0.0
    assert sessions[0].bars[0].adtv_20d == 0.0
