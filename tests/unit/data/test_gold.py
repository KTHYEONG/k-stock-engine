...
def test_materialize_gold_window_uses_in_memory_reader_and_patches_listing_date(tmp_path: Path, monkeypatch) -> None:
    ...
    tables = {
        ...
        SilverTable.SECURITY_MASTER: pl.DataFrame({'instrument_id': ['KRX:1'], 'ticker': ['1'], 'company_id': ['C1'], 'market': ['KOSPI'], 'sector': ['Technology'], 'listing_date': [sessions[0]], 'delisting_date': [None], 'share_class': ['common'], 'status': ['listed'], 'valid_from': [sessions[0]], 'valid_to': [sessions[-1]], 'available_at': [sessions[0]], 'source_hash': [src_hash]}),
