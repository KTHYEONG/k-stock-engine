...
def test_simulate_default_request_matches_canonical_profile(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("src.stocks.workflows.simulate_portfolio.artifact_policy_profile", lambda reg, aid: None)
    ...
    def fake_simulate(snapshot, registry, request, cost_evidence, diagnostics=None):
        ...
        from src.stocks.trading.policy import stock_risk_policy_fingerprint, StockRiskPolicy
    ...
    monkeypatch.setattr("src.stocks.workflows.simulate_portfolio.simulate_portfolio", fake_simulate)
