...
from src.stocks.research.models import ModelManifest, RankICConfig, StableRankComposite
...
def _composite_manifest() -> ModelManifest:
...
def composite_panel(n_instruments: int = 8, n_sessions: int = 14) -> pl.DataFrame:
...
class TestStableRankComposite:
    def test_composite_weights_normalize_or_no_trade(self) -> None:
        labeled = composite_panel()
        splitter = PurgedWalkForward(
            n_folds=3, label_horizon_sessions=2, embargo_sessions=1, session_column="session_index"
        )
        labeled = labeled.with_columns(pl.col("session").rank("dense").cast(pl.Int64).alias("session_index"))
        folds = splitter.split(labeled)
        assert folds
        fold = folds[-1]
        train = labeled[fold.train_mask]
        val = labeled[fold.validation_mask]
        inner = splitter.inner_folds(train)
        ...
        model.fit(train, val, inner_folds=inner)
        if not model.no_trade:
            total = sum(abs(w) for w in model.factor_weights.values())
            assert total == pytest.approx(1.0)
        scored = model.predict(val.drop(["fwd_ret_5d"]))
        assert "pred_score" in scored.columns

    def test_composite_rejects_target_columns_at_predict(self) -> None:
        labeled = composite_panel()
        model = StableRankComposite(
            factors=("rev_5d",),
            manifest=_composite_manifest(),
