...
from legacy.stocks.cli.contracts import TrainCommand, parse_train_command
...
from legacy.stocks.config.runtime import StockRuntimeSettings
from legacy.stocks.data.active import (
    ActiveResearchDataRequest,
    ActiveResearchDataSelection,
    resolve_active_research_data,
)
from legacy.stocks.data.contracts import CoverageRange, DatasetSnapshot
from legacy.stocks.data.costs import load_cost_evidence
from legacy.stocks.data.direct import DirectDataRequest, DirectLoadCheckpoint
from legacy.stocks.data.lineage import ResearchDataBundle, ResolvedDataLineage
from legacy.stocks.data.repositories import (
...
from legacy.stocks.ml.replay_resources import read_host_mem_available_bytes
from legacy.stocks.ml.result_ledger import (
    CostRunContext,
    MlResultLedger,
    MlRunContext,
)
from legacy.stocks.ml.training import TrainingOrchestrator, train_net_alpha_model
from legacy.stocks.observability.contracts import RunDiagnostics, RunIdentity
from legacy.stocks.observability.recorder import open_run_diagnostics
...
from legacy.stocks.settings import REFERENCE_DATE, REFERENCE_DATETIME
...
class TrainSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        module_target: str = "legacy.stocks.cli.train",
        popen: Any | None = None,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--study",
        type=str,
        choices=[e.value for e in __import__("legacy.stocks.cli.contracts", fromlist=["ResearchStudyKind"]).ResearchStudyKind],
        default=None,
...
def run_research_only_economic_family_study(
    ...
    *,
    ...
    """
    if selection is not None:
        from legacy.stocks.ml.contracts import EconomicFamilyStudySettings
        from legacy.stocks.ml.economic_research import evaluate_economic_family_study
...
def run_research_only_return_transfer_study(
    ...
    *,
    ...
    from legacy.stocks.ml.return_transfer import run_research_only_return_transfer_study as _impl
...
def run_research_only_model_selection_study(
    ...
    *,
    ...
    from legacy.stocks.data.ml_integrity import validate_ml_snapshot  # validate_ml_snapshot
    ...
    if selection:
        ...
        from legacy.stocks.ml.model_selection import (
        ...
        try:
            from legacy.stocks.ml.features import stock_net_alpha_v1_contract_book
...
def run_research_only_compound_alpha_study(
    ...
    *,
    ...
    from legacy.stocks.ml.compound_alpha import evaluate_compound_alpha_study
    from legacy.stocks.ml.contracts import CompoundAlphaStudySettings
    ...
    from legacy.stocks.ml import contracts as _ml_contracts
    ...
    settings = CompoundAlphaStudySettings()
    try:
        from legacy.stocks.research.artifacts import ModelArtifactRegistry as _Reg
...
def run_research_only_stock_only_factor_study(
    ...
    *,
    ...
    """
    from legacy.stocks.research.stock_only_factor_study import (
...
def run_research_only_alpha_capacity_audit(
    ...
    *,
    ...
    from legacy.stocks.ml.contracts import AlphaCapacityAuditSettings
...
def _resolve_direct_cost_context(
    ...
    from legacy.stocks.paths import STOCK_CATALOG_ROOT
    from legacy.stocks.data.costs import load_cost_evidence
...
def _dispatch_train_command(command: TrainCommand) -> int:
    ...
    from legacy.stocks.observability.contracts import RunIdentity
    from legacy.stocks.observability.recorder import open_run_diagnostics
    ...
    try:
        evidence = load_cost_evidence(selection.cost_evidence_path, __import__("legacy.stocks.data.contracts", fromlist=["CoverageRange"]).CoverageRange(start=command.active_request.start, end=command.active_request.end))
...
def main(args: list[str] | None = None) -> int:
    ...
    if parsed.as_of:
        from legacy.stocks.data.catalog import CatalogStore
        from legacy.stocks.data.lineage import (
        ...
        try:
            ...
            if "feature_set mismatch" in message or "net-alpha" in message:
                raise ValueError(
                    ...
                    "`python -m legacy.stocks.cli.build_research --pipeline net-alpha "
