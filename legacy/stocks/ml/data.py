...
from legacy.stocks.data.direct import MlMarketData
...
def _reject_feature_set(snapshot: DatasetSnapshot | ResearchDataBundle) -> None:
    ...
    if feature_set != CANONICAL_FEATURE_SET:
        raise ValueError(
            ...
            "net-alpha snapshot via `python -m legacy.stocks.cli.build_research "
            "--pipeline net-alpha`."
        )
    ...
    for horizon in candidate_horizon_sessions:
        if long_format:
            ...
            if "gross_return" in subset.columns:
                from legacy.stocks.ml.labels import GROSS_COLUMN as _GROSS_COL
            ...
            if gross_wide in frame.columns:
                from legacy.stocks.ml.labels import GROSS_COLUMN as _GROSS_COL2

                select_columns.append(pl.col(gross_wide).alias(_GROSS_COL2))
            elif "gross_return" in frame.columns:
                from legacy.stocks.ml.labels import GROSS_COLUMN as _GROSS_COL3
