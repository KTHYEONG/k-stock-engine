...
from legacy.stocks.ml.wealth_transfer import WealthEvidenceKind, evaluate_wealth_candidate
...
from legacy.stocks.ml.economic_objective import route_training_target
from legacy.stocks.ml.family_specs import family_feature_columns, family_spec, fit_family_model
from legacy.stocks.ml.models import SCORE_COLUMN
# wiring imports for spec compliance
try:
    from legacy.stocks.ml.training import build_initial_calibration_seed  # build_initial_calibration_seed
...
from legacy.stocks.research.folds import Fold, PurgedWalkForward
# wiring: prepare_screening_fold_cache, fit_model_family_oof
from legacy.stocks.ml.model_selection_screening import sample_labeled_screen_rows as _leaf_sample_labeled_screen_rows
from legacy.stocks.ml.model_selection_plan import resolve_model_selection_plan as _leaf_resolve_model_selection_plan
from legacy.stocks.ml.model_selection_statistics import segmented_moving_block_lower_bound as _leaf_segmented_moving_block_lower_bound
...
def _build_prepared_screen_sample(
    ...
    try:
        ...
        try:
            from legacy.stocks.ml.economic_objective import project_route_utility
...
def preflight_model_selection_inputs(
    ...
    from legacy.stocks.ml.contracts import ModelSelectionInputPreflight
    ...
    try:
        from legacy.stocks.ml.economic_objective import project_route_utility, route_training_target
...
def build_model_selection_study_settings(parsed: object, request: NetAlphaTrainingRequest) -> ModelSelectionStudySettings:
    ...
    if hasattr(parsed, "candidate_training_lookback_sessions"):
        ...
        try:
            from legacy.stocks.cli.train import _parse_training_lookback_candidates
...
def route_calibration_ledger(oof_labels: pl.DataFrame, request: NetAlphaTrainingRequest) -> pl.DataFrame:
    ...
    from legacy.stocks.ml.labels import GROSS_COLUMN, RISK_RESIDUAL_COLUMN
...
def build_initial_calibration_seed(matrix, initial_train_rows, request, horizon_sessions, base_manifest, *, data, family, training_top_k=None):  # type: ignore
    from legacy.stocks.ml.training import build_initial_calibration_seed as _real_build
...
def _causal_oof_calibrate(oof, oof_labels, request, horizon_sessions, *, seed_ledger=None):  # type: ignore
    from legacy.stocks.ml.training import _causal_oof_calibrate as _real_cal
...
def _aligned_screen_labels(
    ...
    from legacy.stocks.ml.labels import GROSS_COLUMN as _GC_ALIGNED
...
def prepare_screening_fold_cache(
    ...
    *,
    ...
    from legacy.stocks.ml.features import materialize_model_feature_sources
...
def _screen_route_utility_series(
    ...
    *,
    ...
    from legacy.stocks.ml.labels import GROSS_COLUMN
    from legacy.stocks.ml.economic_objective import project_route_utility
...
def _screen_model_family_legacy(
    ...
    try:
        from legacy.stocks.ml.labels import GROSS_COLUMN as _GC
    except Exception:
        _GC = "gross_return"
    route_kind = str(getattr(getattr(request, "route_objective", None), "kind", "unhedged_absolute"))
    try:
...
def _build_label_join(data: NetAlphaResearchData, horizon: int) -> pl.DataFrame:
    ...
    from legacy.stocks.ml.training import _build_label_join as _orig
...
def _sync_legacy_selection_hooks() -> None:
    ...
    facade = sys.modules.get("legacy.stocks.ml.model_selection")
    if facade is None:
        return
    for name in (
...
def evaluate_model_selection_study(
    ...
    from legacy.stocks.ml.window_research import derive_study_fold_count
    ...
    for ev in screen_evidence:
        ...
        try:
            from legacy.stocks.ml.contracts import RiskSettings as RiskSettingsLocal  # noqa: N814
            from legacy.stocks.ml.training import _causal_oof_calibrate, _replay_costs_batch
...
def run_research_only_model_selection_study(parsed, request):  # type: ignore[no-redef]
    ...
    from legacy.stocks.cli.train import run_research_only_model_selection_study as _cli_impl
