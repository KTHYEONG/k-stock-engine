...
from legacy.stocks.data.ml_integrity import validate_ml_snapshot
...
from legacy.stocks.ml.data import assess_snapshot_outcome_readiness
from legacy.stocks.ml.discovery import discover_horizons
...
from legacy.stocks.ml.fitting import (
...
from legacy.stocks.ml.preparation import (
    PreparedFold,
...
from legacy.stocks.ml.wealth_transfer import evaluate_wealth_candidate
from legacy.stocks.ml.training_temporal import _index_sessions as _leaf_index_sessions
from legacy.stocks.ml.training_temporal import _locked_holdout as _leaf_locked_holdout
...
try:
    from legacy.stocks.ml.model_selection import route_calibration_ledger  # route_calibration_ledger
...
if route_calibration_ledger is None:  # type: ignore
    def route_calibration_ledger(oof_labels: pl.DataFrame, request: NetAlphaTrainingRequest) -> pl.DataFrame:  # type: ignore
        ...
        return res
from legacy.stocks.ml.satellite_overlay import (
...
from legacy.stocks.ml.telemetry import peak_rss_mib as _peak_rss_mib
from legacy.stocks.ml.telemetry import (
...
from legacy.stocks.research.calibration_schedule import SessionClusterCalibrationSchedule
...
from legacy.stocks.research.models import Model, ModelManifest
    ...
    def run(self) -> ModelManifest:
        ...
        if self.data.manifest.feature_set != CANONICAL_FEATURE_SET:
            raise ValueError(
                ...
                "snapshot via `python -m legacy.stocks.cli.build_research "
                "--pipeline net-alpha`."
            )
    ...
    *,
    ...
    if data.manifest.feature_set != CANONICAL_FEATURE_SET:
        raise ValueError(
            ...
            "via `python -m legacy.stocks.cli.build_research --pipeline net-alpha`."
        )
...
def _select_elastic_alpha(
    ...
    """
    del manifest
    from legacy.stocks.ml.fitting import _select_elastic_alpha_prepared
    from legacy.stocks.ml.labels import REALIZED_RETURN_COLUMN, TARGET_COLUMN
...
def _execution_replay_context(
    ...
    *,
    ...
    from legacy.stocks.ml.contracts import RouteObjective
...
def _rank_ic_lower_bound(
    ...
    if n < 2:
        return 0.0
    from legacy.stocks.ml.horizons import _segment_block_length
...
def build_initial_calibration_seed(
    ...
    *,
    ...
    """
    from legacy.stocks.ml.fitting import OofFitRequest
    from legacy.stocks.ml.fitting import fit_horizon_oof as _fit_horizon_oof
...
def _causal_oof_calibrate(
    ...
    *,
    ...
    from legacy.stocks.ml.labels import GROSS_COLUMN as _GC_FB
...
def _fit_oof(
    ...
    *,
    ...
    if family == TAIL_LAMBDARANK_FAMILY:
        from legacy.stocks.ml.economic_research import fit_tail_lambdarank_oof
...
def _adopt_model_family(
    ...
    *,
    ...
    from legacy.stocks.ml.horizons import _cohort_bootstrap
...
def _run_model_selection_mainline(
    ...
    if not getattr(request, "model_selection_mainline", False) and not getattr(request, "enable_model_selection_mainline", False):  # noqa: SIM102
        return None
    from legacy.stocks.ml.contracts import ModelSelectionStudySettings
...
def _growth_route_projection(
    ...
    *,
    ...
    if executable_overlay_data is not None and capital_plan_settings is not None and account_certification is not None and compounding is not None:
        try:
            from legacy.stocks.ml.hedge_sleeve import build_executable_hedge_evidence, certify_executable_hedged_growth_route
