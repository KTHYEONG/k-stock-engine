...
from legacy.stocks.backtesting.contracts import (
...
from legacy.stocks.backtesting.execution import (
    PreparedExecutionState as _ScenarioState,
)
from legacy.stocks.backtesting.market import (
    PreparedReplayMarket,
    _as_datetime,
)
from legacy.stocks.backtesting.metrics import build_backtest_attribution
from legacy.stocks.data.contracts import DatasetSnapshot
from legacy.stocks.data.costs import (
    CostEvidence,
    krx_market_for_code,
    resolve_fill_cost,
)
from legacy.stocks.data.lineage import ResearchDataBundle
...
BacktesterPlanner = Callable[..., TradingCycleResult]
...
class StockBacktester:
    ...
    def run(
        ...
        *,
        ...
        # Branch before any materialization: the no-trade and prebuilt-prepared
        # branches never call ``partition_by``; only the DataFrame reference
        # fallback pays for per-session partitioning.
