...
from legacy.stocks.data.direct import (
    DirectDataRequest,
...
from legacy.stocks.ml.data import validate_ml_market_data
...
def test_direct_loader_rejects_non_monotonic_sessions() -> None:
    ...
    from legacy.stocks.data.direct import _validate_direct_frame
...
def test_v6_simulation_rejects_divergent_feature_content_hash(tmp_path: Path) -> None:
    ...
    from legacy.stocks.workflows.simulate_portfolio import simulate_portfolio
...
def test_terminal_obs_02_join_preflight_fails_closed(tmp_path: Path) -> None:
    ...
    from legacy.stocks.data.direct import DirectLoadCheckpoint
