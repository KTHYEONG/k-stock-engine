...
def test_stock_only_cli_requires_small_account_and_rejects_etf_satellite() -> None:
    from legacy.stocks.cli.train import _build_training_request, build_parser
...
def test_train_main_resolves_active_selection_once(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("legacy.stocks.ml.training.train_net_alpha_model", fake_train)
...
def test_train_cli_rejects_conflicting_studies(monkeypatch) -> None:
    from legacy.stocks.cli.contracts import parse_train_command
...
def test_model_selection_uses_active_selection_once_and_records_reproducible_inputs(monkeypatch, tmp_path) -> None:
    ...
    from legacy.stocks.data.active import ActiveResearchDataRequest, ActiveResearchDataSelection
    from legacy.stocks.data.direct import DirectDataRequest
    import hashlib, json as _json
    from legacy.stocks.data.catalog import ActiveDatasetPolicy, CatalogEntry, CatalogKind, CatalogStore, EvidenceCompleteness
    from legacy.stocks.data.contracts import CoverageRange
    from src.storage.parquet_datasets import ParquetDatasetStore, canonical_content_hash
    ...
    monkeypatch.setattr("legacy.stocks.cli.train.MlComparisonReport", FakeLedger)
    ...
    monkeypatch.setattr("legacy.stocks.cli.train.load_cost_evidence", fake_load_cost)
    ...
    monkeypatch.setattr("legacy.stocks.ml.model_selection.evaluate_model_selection_study", fake_evaluate)
    monkeypatch.setattr("legacy.stocks.ml.model_selection.build_model_selection_study_settings", lambda parsed, req: SimpleNamespace())
...
def test_train_cli_exposes_stock_only_factor_study_and_conflicts_fail_before_io() -> None:
    import pytest
    from legacy.stocks.cli.contracts import ResearchStudyKind, parse_train_command
    from legacy.stocks.cli.train import build_parser
