...
class TestStreamingReplay:
    ...
    def test_streaming_reuses_supplied_prepared_batch(
        ...
        context = ExecutionReplayContext(
            registry=__import__("legacy.stocks.research.artifacts", fromlist=["ModelArtifactRegistry"]).ModelArtifactRegistry(
            ...
            risk_policy=__import__("legacy.stocks.ml.training", fromlist=["_risk_policy_for_profile"])._risk_policy_for_profile(
                request, profile, 10,
            ...
            execution_policy=__import__("legacy.stocks.domain.execution_policy", fromlist=["SCHEDULED_OPEN_V1"]).SCHEDULED_OPEN_V1,
            seed=42,
        )
        ...
        monkeypatch.setattr(
            "legacy.stocks.ml.execution_replay.prepare_execution_replay_batch",
            fail_if_prepared,
        )
...
class TestReplayStream01:
    ...
    def test_stream_builds_each_segment_once_with_single_live_segment(self) -> None:
        ...
        from legacy.stocks.ml.execution_replay import ExecutionReplayContext
