...
def test_MODEL_SELECTION_01_CATALOGUE_REJECTS_DUPLICATE_BOOSTERS():
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionStudySettings, DEFAULT_MODEL_SELECTION_FAMILIES, DECLARED_MODEL_SELECTION_FAMILIES
...
def test_MODEL_SELECTION_03_REAL_OOF_OR_REJECTION():
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionCandidate, NetAlphaTrainingRequest
    from legacy.stocks.ml.model_selection import fit_model_family_oof
...
def test_MODEL_SELECTION_04_LEDGER_DERIVED_ECONOMICS():
    ...
    from legacy.stocks.ml.contracts import ModelSelectionStudySettings, NetAlphaTrainingRequest
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
...
def test_MODEL_SELECTION_05_ENSEMBLE_INCREMENTAL_EVIDENCE():
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionCandidate, FeatureAttributionEvidence, ModelSelectionStudySettings
    from legacy.stocks.ml.model_selection import select_diversified_ensemble
...
def test_MODEL_SELECTION_FAST_01_CACHE_SHARED(monkeypatch):
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionComputeBudget
    from legacy.stocks.ml.model_selection import prepare_screening_fold_cache, screen_model_family, deterministic_screen_sample_rows
    from legacy.stocks.ml.features import fit_research_feature_schema, materialize_model_feature_sources
    import legacy.stocks.ml.model_selection as msel
    ...
    from legacy.stocks.research.folds import PurgedWalkForward
    panel = _index_sessions(frame)
    req_dummy = __import__("legacy.stocks.ml.contracts", fromlist=["NetAlphaTrainingRequest"]).NetAlphaTrainingRequest(artifact_id="cache01", candidate_horizon_sessions=(10,))
    ...
    monkeypatch.setattr("legacy.stocks.ml.features.fit_research_feature_schema", counted_fit)
    monkeypatch.setattr("legacy.stocks.ml.features.materialize_model_feature_sources", counted_mat)
    caches = []
    for fold in folds:
...
def test_MODEL_SELECTION_FAST_02_SCREEN_ALL_FULL_TWO(monkeypatch):
    from legacy.stocks.ml.contracts import ModelSelectionStudySettings, NetAlphaTrainingRequest, ModelSelectionComputeBudget, ModelFamily
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
...
def test_MODEL_SELECTION_FAST_03_PIT_SAMPLE():
    import polars as pl
    from legacy.stocks.ml.model_selection import deterministic_screen_sample_rows
...
def test_MODEL_SELECTION_FAST_04_BUDGET_FAIL_CLOSED():
    from legacy.stocks.ml.contracts import NetAlphaTrainingRequest, ModelSelectionStudySettings, ModelSelectionComputeBudget, ModelFamily
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
...
def test_PROFILED_ML_SELECTION_01_ALIGNED_SCREEN_PANEL():
    import numpy as np, polars as pl
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionComputeBudget
    from legacy.stocks.ml.model_selection import prepare_screening_fold_cache, screen_model_family
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles
    from legacy.stocks.research.folds import PurgedWalkForward
    from legacy.stocks.ml.training import _index_sessions, _locked_holdout
    from datetime import datetime, UTC, timedelta
    ...
    panel = _index_sessions(frame)
    req = __import__("legacy.stocks.ml.contracts", fromlist=["NetAlphaTrainingRequest"]).NetAlphaTrainingRequest(artifact_id="aligned01", candidate_horizon_sessions=(10,))
    ...
    aligned_short = __import__("legacy.stocks.ml.model_selection", fromlist=["_aligned_screen_labels"])._aligned_screen_labels(
...
def test_PROFILED_ML_SELECTION_03_NO_PERMUTATION_REFIT(monkeypatch):
    import polars as pl, numpy as np
    from legacy.stocks.ml.contracts import ModelSelectionComputeBudget, ModelFamily
    from legacy.stocks.ml.model_selection import prepare_screening_fold_cache, screen_model_family
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles
    from legacy.stocks.research.folds import PurgedWalkForward
    from legacy.stocks.ml.training import _index_sessions, _locked_holdout
    ...
    frame=pl.DataFrame(rows)
    panel=_index_sessions(frame)
    req=__import__("legacy.stocks.ml.contracts", fromlist=["NetAlphaTrainingRequest"]).NetAlphaTrainingRequest(artifact_id="norefit", candidate_horizon_sessions=(10,))
    pre,_,_=_locked_holdout(panel, req)
    ...
    class CountET(orig_et):
        def fit(self, *a, **kw):
            fit_calls["fit"]+=1
            return super().fit(*a, **kw)
    monkeypatch.setattr("legacy.stocks.ml.model_selection.ElasticNet", CountEN)
    monkeypatch.setattr("legacy.stocks.ml.model_selection.ExtraTreesRegressor", CountET)
    # also patch lgb train to count
    orig_lgb_train = msel.lgb.train
    def counted_lgb(*a, **kw):
        fit_calls["fit"]+=1
        return orig_lgb_train(*a, **kw)
    monkeypatch.setattr("legacy.stocks.ml.model_selection.lgb.train", counted_lgb)
...
def test_PROFILED_ML_SELECTION_04_REQUESTED_FOLD_CAP():
    import polars as pl, numpy as np
    from legacy.stocks.ml.contracts import NetAlphaTrainingRequest, ModelSelectionStudySettings, ModelSelectionComputeBudget
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
    from datetime import datetime, UTC, timedelta
    import tempfile, pathlib
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles as _roles_fn
    _roles = _roles_fn()
...
def test_PROFILED_ML_SELECTION_05_BUDGET_FAIL_CLOSED():
    ...
    from tests.fixtures.stocks.helpers import stock_liquidity_model
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles as _roles_fn2
    _roles2=_roles_fn2()
...
def test_MLCMP_LOG_04():  # noqa: N802
    ...
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionStudySettings, NetAlphaTrainingRequest, ModelSelectionComputeBudget
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
    ...
    from legacy.stocks.ml.contracts import FeatureAttributionEvidence, FamilyScreenEvidence
    orig_screen = None
    try:
        from legacy.stocks.ml.model_selection import screen_model_family as _orig_screen
    ...
    def _force_positive_screen(cache, label_join, family, budget, deadline, *args, **kwargs):
        ...
        if orig_screen is not None:
            ...
            if ev is not None and str(family) == "elastic_net_v2":
                ...
                from legacy.stocks.ml.contracts import ScreenEconomicEvidence
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        import legacy.stocks.ml.model_selection as msel_mod
        ...
        with _patch_screen("legacy.stocks.ml.model_selection.screen_model_family", side_effect=_force_positive_screen):
            with patch.object(msel_mod.logger, "debug", side_effect=capture_debug):
                with patch("legacy.stocks.ml.training._causal_oof_calibrate", side_effect=RuntimeError("calib boom")):
                    result = evaluate_model_selection_study(data, request, settings, registry=registry)
                    if captured:
                        ...
                        return
                    captured.clear()
                    with patch("legacy.stocks.ml.training._replay_costs_batch", side_effect=ValueError("replay boom")):
...
def _route_screen_request():
    ...
    from legacy.stocks.ml.contracts import ExecutionFrontierSettings, NetAlphaTrainingRequest
...
def test_SCENARIO_ML_ROUTE_01_UNHEDGED_GROSS_ONCE():
    import pytest
    from legacy.stocks.ml.model_selection import _screen_prefix_economic_evidence
...
def test_SCENARIO_ML_ADMISSION_01_NEGATIVE_SCREEN(monkeypatch):
    ...
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionStudySettings, ModelSelectionComputeBudget, FeatureAttributionEvidence, NetAlphaTrainingRequest
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
    ...
    import legacy.stocks.ml.model_selection as _msel_for_neg
    ...
    monkeypatch.setattr("legacy.stocks.ml.model_selection.prepare_screening_fold_cache", _fake_prepare_neg)
    ...
    def fake_screen(cache, label_join, family, budget, deadline):
        ...
        return __import__("legacy.stocks.ml.contracts", fromlist=["FamilyScreenEvidence"]).FamilyScreenEvidence(family=family, screen_lower_bound=lb, screen_se=0.005, attribution=attr, qualified_for_full_oof=False, selected_family=False)

    monkeypatch.setattr("legacy.stocks.ml.model_selection.screen_model_family", fake_screen)
    ...
    def counted_fit(*a, **kw):
        oof_calls["n"]+=1
        return orig_fit(*a, **kw)
    monkeypatch.setattr("legacy.stocks.ml.model_selection.fit_model_family_oof", counted_fit)
    replay_calls={"n":0}
    # patch replay to count if called
    monkeypatch.setattr("legacy.stocks.ml.training._replay_costs_batch", lambda *a, **kw: (_ for _ in ()).throw(AssertionError("should not be called")) if False else {})
...
def test_SCENARIO_ML_ADMISSION_02_POSITIVE_ONE_SE(monkeypatch):
    ...
    settings=ModelSelectionStudySettings(candidate_lookback_sessions=(504,), candidate_families=tuple(ModelFamily.__members__.values()), common_min_train_sessions=504, min_validation_segment_sessions=5, compute_budget=ModelSelectionComputeBudget(wall_clock_seconds=30.0, screen_phase_seconds=20.0, max_full_replay_families=2))
    import legacy.stocks.ml.model_selection as _msel_for_pos
    ...
    monkeypatch.setattr("legacy.stocks.ml.model_selection.prepare_screening_fold_cache", _fake_prepare_pos)
    ...
    monkeypatch.setattr("legacy.stocks.ml.model_selection.fit_model_family_oof", fake_fit)
...
def test_SCENARIO_ML_OOF_03_COMPLETE_FOLDS(monkeypatch):
    ...
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionCandidate, FeatureAttributionEvidence, NetAlphaTrainingRequest, ModelSelectionStudySettings, ModelSelectionComputeBudget
    from legacy.stocks.research.folds import PurgedWalkForward
    from legacy.stocks.ml.training import _index_sessions, _locked_holdout
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles, materialize_model_feature_sources, fit_research_feature_schema, apply_research_feature_schema
    from datetime import datetime, UTC, timedelta
    from src.core.datasets import DatasetManifest
    from src.core.instruments import AssetKind
    from legacy.stocks.ml.contracts import NetAlphaResearchData
    ...
    monkeypatch.setattr("legacy.stocks.ml.model_selection._fit_one_fold", failing_fit)
    # also patch replay to ensure not called if we were to go through study (but we test direct fit)
    replay_calls={"n":0}
    monkeypatch.setattr("legacy.stocks.ml.training._replay_costs_batch", lambda *a, **kw: (replay_calls.__setitem__("n", replay_calls["n"]+1), {} )[1])
    from legacy.stocks.ml.model_selection import fit_model_family_oof
    ...
    import tempfile, pathlib
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from legacy.stocks.ml.contracts import ModelSelectionStudySettings, ModelSelectionComputeBudget
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    ...
    def fake_screen(cache, label_join, family, budget, deadline):
        ...
        se=0.01
        from legacy.stocks.ml.contracts import FamilyScreenEvidence
        return FamilyScreenEvidence(family=family, screen_lower_bound=lb, screen_se=se, attribution=attr, qualified_for_full_oof=False, selected_family=False)
    monkeypatch.setattr("legacy.stocks.ml.model_selection.screen_model_family", fake_screen)
    ...
    def always_fail_fit(*a, **kw):
        import polars as pl
        return pl.DataFrame(), pl.DataFrame()
    monkeypatch.setattr("legacy.stocks.ml.model_selection.fit_model_family_oof", always_fail_fit)
    # track replay
    replay_cnt={"n":0}
    orig_replay=__import__("legacy.stocks.ml.training", fromlist=["_replay_costs_batch"]). _replay_costs_batch
    def counted_replay(*a, **kw):
        replay_cnt["n"]+=1
        return {}
    monkeypatch.setattr("legacy.stocks.ml.training._replay_costs_batch", counted_replay)
    with tempfile.TemporaryDirectory() as tmp:
        registry=ModelArtifactRegistry(pathlib.Path(tmp))
...
def test_SCENARIO_ML_DATA_04_LINEAR_IMPUTATION():
    ...
    import numpy as np, polars as pl
    from legacy.stocks.ml.model_selection import _impute_and_standardize_from_train, _fit_one_fold
    from legacy.stocks.ml.contracts import ModelFamily, FeatureAttributionEvidence
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles, materialize_model_feature_sources, fit_research_feature_schema, apply_research_feature_schema
    from legacy.stocks.ml.labels import SESSION_COLUMN, TARGET_COLUMN, REALIZED_RETURN_COLUMN, REFERENCE_COST_COLUMN
    ...
    from legacy.stocks.ml.training import _index_sessions
    train=frame.head(60)
    ...
    from legacy.stocks.ml.features import materialize_model_feature_sources, fit_research_feature_schema
...
def test_SCENARIO_ML_RUNTIME_05_REUSE_SCREEN_ATTRIBUTION(monkeypatch):
    ...
    import polars as pl, numpy as np
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionCandidate, FeatureAttributionEvidence, NetAlphaTrainingRequest
    from legacy.stocks.research.folds import PurgedWalkForward
    from legacy.stocks.ml.training import _index_sessions, _locked_holdout
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles, materialize_model_feature_sources, fit_research_feature_schema
    from datetime import datetime, UTC, timedelta
    from src.core.datasets import DatasetManifest
    from src.core.instruments import AssetKind
    from legacy.stocks.ml.contracts import NetAlphaResearchData
    ...
    monkeypatch.setattr("legacy.stocks.ml.model_selection.select_feature_groups", counted_select)
    ...
    def capturing_fit(train, validation, family, schema, selected):
        ...
        return orig_fit_one(train, validation, family, schema, selected)
    monkeypatch.setattr("legacy.stocks.ml.model_selection._fit_one_fold", capturing_fit)
    from legacy.stocks.ml.model_selection import fit_model_family_oof
    oof, labs = fit_model_family_oof(pre, folds, data, request, cand, fold_attributions=tuple(fold_attrs), deadline_monotonic=None)
    assert call_cnt["n"] == 0
...
def test_SCENARIO_ML_RUNTIME_06_DEADLINE_LEDGER(monkeypatch):
    ...
    def fake_monotonic():
        ...
        if call_counter["n"] <= 100:
            ...
            return start + 10.0
    monkeypatch.setattr("legacy.stocks.ml.model_selection.time.monotonic", fake_monotonic)
...
def test_evaluate_model_selection_study_uses_resolved_reference_cell_not_frontier_order(monkeypatch):
    import tempfile, pathlib, polars as pl, numpy as np
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import ExecutionFrontierSettings, ModelFamily, ModelSelectionStudySettings, ModelSelectionComputeBudget, NetAlphaTrainingRequest, DEFAULT_POLICY_PROFILES, FeatureAttributionEvidence, FamilyScreenEvidence
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
    ...
    monkeypatch.setattr("legacy.stocks.ml.training._replay_costs_batch", fake_replay_batch)
    # also patch model_selection import path for same symbol
    monkeypatch.setattr("legacy.stocks.ml.model_selection._replay_costs_batch", fake_replay_batch, raising=False)
...
def test_evaluate_model_selection_study_replays_each_registered_profile_after_single_oof(monkeypatch):
    import tempfile, pathlib, polars as pl, numpy as np
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import ExecutionFrontierSettings, ModelFamily, ModelSelectionStudySettings, ModelSelectionComputeBudget, NetAlphaTrainingRequest, FeatureAttributionEvidence, FamilyScreenEvidence
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
...
def test_evaluate_model_selection_study_admits_zero_band_profile_only_with_positive_execution_evidence(monkeypatch):
    import tempfile, pathlib, polars as pl, numpy as np
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import ExecutionFrontierSettings, ModelFamily, ModelSelectionStudySettings, ModelSelectionComputeBudget, NetAlphaTrainingRequest, FeatureAttributionEvidence, FamilyScreenEvidence, LEGACY_OVERLAY_PROFILE_ID, LOWER_BOUND_ONLY_PROFILE_ID
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
...
def test_evaluate_model_selection_study_preserves_profile_inclusive_alpha_budget():
    from legacy.stocks.ml.contracts import ExecutionFrontierSettings, NetAlphaTrainingRequest, ModelSelectionStudySettings, ModelSelectionComputeBudget
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
...
def test_full_oof_uses_canonical_family_fit_signature(monkeypatch):
    ...
    from legacy.stocks.ml.contracts import ModelFamily, FeatureAttributionEvidence
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles, materialize_model_feature_sources, fit_research_feature_schema
    from legacy.stocks.ml.model_selection import _fit_one_fold
    from legacy.stocks.ml.labels import SESSION_COLUMN
    rng = np.random.default_rng(0)
    _roles = stock_net_alpha_v1_roles()
    ...
    calls = {}
    import legacy.stocks.ml.model_selection as msel
    import legacy.stocks.ml.family_specs as fspec
    ...
    monkeypatch.setattr("legacy.stocks.ml.model_selection.fit_family_model", spy_fit)
    # also ensure family_spec used
    for fam in [ModelFamily.elastic_net_v2, ModelFamily.extra_trees_v1]:
        from legacy.stocks.ml.contracts import ModelSelectionCandidate, FeatureAttributionEvidence
        ...
        cand = __import__("legacy.stocks.ml.contracts", fromlist=["ModelSelectionCandidate"]).ModelSelectionCandidate(candidate_id=f"{fam.value}_h10", family=fam, horizon_sessions=10, selected_source_groups=selected, oof_fingerprint="fp", attribution=(attr,), training_top_k=None)
        preds = _fit_one_fold(train_frame, valid_frame, cand, schema, selected, request=__import__("legacy.stocks.ml.contracts", fromlist=["NetAlphaTrainingRequest"]).NetAlphaTrainingRequest(artifact_id="sigtest", candidate_horizon_sessions=(10,)))
...
def test_lambdarank_uses_resolved_execution_k_and_exact_k_relevance():
    import polars as pl, numpy as np
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import ExecutionFrontierSettings, NetAlphaTrainingRequest, ModelSelectionStudySettings, ModelFamily, DEFAULT_POLICY_PROFILES
    from legacy.stocks.ml.model_selection import resolve_model_selection_plan, _fit_one_fold
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles, materialize_model_feature_sources, fit_research_feature_schema
    ...
    from legacy.stocks.ml.contracts import FeatureAttributionEvidence, ModelSelectionCandidate
    _roles = stock_net_alpha_v1_roles()
    rng = np.random.default_rng(1)
...
def test_research_only_study_derives_single_reference_cell_from_bound_request(monkeypatch):
    import tempfile, pathlib, polars as pl, numpy as np
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import ExecutionFrontierSettings, NetAlphaTrainingRequest, ModelSelectionStudySettings, ModelFamily
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study, resolve_model_selection_plan
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles
...
def test_route_calibration_preserves_fail_closed_no_fill_gate(monkeypatch):
    import tempfile, pathlib, polars as pl, numpy as np
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import NetAlphaTrainingRequest, ModelSelectionStudySettings, ModelSelectionComputeBudget, ModelFamily, ExecutionFrontierSettings
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
    ...
    from src.core.datasets import DatasetManifest
    from src.core.instruments import AssetKind
    from legacy.stocks.ml.contracts import NetAlphaResearchData, FeatureAttributionEvidence, FamilyScreenEvidence
    _roles = stock_net_alpha_v1_roles()
    rng = np.random.default_rng(3)
    sessions=[datetime(2024,1,1,tzinfo=UTC)+timedelta(days=i) for i in range(800)]
    rows=[]
    ...
    import legacy.stocks.ml.training as tmod
    ...
    def fake_replay(registry, calibrated, labels_, win_req, horizon, risk, pre_holdout, manifest_, specs):
        ...
        for (c,k,prof) in specs:
            ...
            out[(horizon,c,k,prof.profile_id)] = ProfileReplayEvidence(candidate=ev, dense_shadow=None)
        return out
    monkeypatch.setattr("legacy.stocks.ml.training._replay_costs_batch", fake_replay)
    with tempfile.TemporaryDirectory() as tmp:
        registry=ModelArtifactRegistry(pathlib.Path(tmp))
        result=evaluate_model_selection_study(data, request, settings, registry=registry)
        assert result["selected_family"] is None
...
def test_ML_SCREEN_01_cross_section_and_budget_gate() -> None:
    ...
    import polars as pl
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import ScreenSamplingPlan, ScreenSamplingEvidence, ModelSelectionComputeBudget
    from legacy.stocks.ml.model_selection import deterministic_screen_sample_rows, _select_inner_feature_groups
    from legacy.stocks.ml.contracts import ModelFamily, NetAlphaTrainingRequest, RouteObjective, RouteObjectiveKind
...
def test_ML_WINDOW_01_locked_holdout_and_outer_validation_calendar() -> None:
    ...
    from datetime import datetime, UTC, timedelta
    import numpy as np
    from legacy.stocks.ml.contracts import NetAlphaTrainingRequest, ModelSelectionStudySettings
    from legacy.stocks.ml.training import _index_sessions, _locked_holdout
    from legacy.stocks.research.folds import PurgedWalkForward
...
def test_nested_feature_selection_ignores_outer_validation_mutation():
    import polars as pl, numpy as np
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import NetAlphaTrainingRequest, ScreenSamplingPlan, ModelFamily, ExecutionFrontierSettings
    from legacy.stocks.ml.model_selection import _select_inner_feature_groups
    rng = np.random.default_rng(0)
    sessions = [datetime(2024, 1, 1, tzinfo=UTC) + timedelta(days=i) for i in range(30)]
    ...
    from legacy.stocks.ml.labels import TARGET_COLUMN, REFERENCE_COST_COLUMN
    outer_rows2 = []
    for s in sessions[:20]:
        for t in range(10):
            outer_rows2.append({
...
def test_screen_bootstraps_pooled_decision_utilities_not_fold_bounds():
    import numpy as np
    from legacy.stocks.ml.contracts import ScreenRouteUtilitySeries
    from legacy.stocks.ml.model_selection import _aggregate_screen_route_evidence
    from legacy.stocks.research.bootstrap import pooled_segment_bootstrap_means
    rng = np.random.default_rng(42)
...
def test_confidence_plan_applies_multiplicity_only_at_final_admission(monkeypatch):
    from legacy.stocks.ml.contracts import NetAlphaTrainingRequest, ModelSelectionStudySettings, ExecutionFrontierSettings
    from legacy.stocks.ml.model_selection import resolve_study_confidence_plan
...
def test_conversion_modes_reuse_one_oof_score_frame(monkeypatch):
    ...
    from legacy.stocks.config.research import policy_profiles_with_continuous_uncertainty, DEFAULT_POLICY_PROFILES
...
def test_screen_route_diagnostic_preserves_rejection(monkeypatch):
    ...
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.model_selection import ScreenRouteDiagnostic, prepare_screening_fold_cache, screen_model_family
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionComputeBudget, NetAlphaTrainingRequest, ExecutionFrontierSettings
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles
    from legacy.stocks.research.folds import PurgedWalkForward
    from legacy.stocks.ml.training import _index_sessions, _locked_holdout
    import pytest
    ...
    monkeypatch.setattr("legacy.stocks.ml.model_selection.family_training_profile", boom)
...
def test_pooled_decision_capacity_allows_subminimum_folds():
    ...
    import polars as pl
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles
    from legacy.stocks.ml.model_selection import prepare_screening_fold_cache
    from legacy.stocks.ml.contracts import ModelSelectionComputeBudget
...
def test_pooled_decision_capacity_rejects_before_learner_fit(monkeypatch):
    ...
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles
    from legacy.stocks.ml.contracts import NetAlphaResearchData, NetAlphaTrainingRequest, ModelSelectionStudySettings, ModelSelectionComputeBudget, ExecutionFrontierSettings, ModelFamily
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study, prepare_screening_fold_cache
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.datasets import DatasetManifest
    from src.core.instruments import AssetKind
    from src.core.costs import default_base_schedule, default_stress_schedule
    ...
    monkeypatch.setattr("legacy.stocks.ml.model_selection.prepare_screening_fold_cache", fake_prepare)
    with tempfile.TemporaryDirectory() as tmp:
        registry = ModelArtifactRegistry(pathlib.Path(tmp))
        result = evaluate_model_selection_study(data, request, settings, registry=registry)
        assert result["status"] == "RESEARCH_ONLY"
...
def test_pooled_route_utility_below_minimum_rejects_family(monkeypatch):
    ...
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles
    from legacy.stocks.ml.contracts import NetAlphaResearchData, NetAlphaTrainingRequest, ModelSelectionStudySettings, ModelSelectionComputeBudget, ExecutionFrontierSettings, ModelFamily, ScreenRouteUtilitySeries, FamilyScreenEvidence, FeatureAttributionEvidence, ScreenEconomicEvidence
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.datasets import DatasetManifest
    from src.core.instruments import AssetKind
    ...
    def fake_screen(cache, label_join, family, budget, deadline, request=None, bootstrap_alpha=None, bootstrap_resamples=None, horizon_sessions=None, rebalance_frequency_sessions=None, execution_top_k=None, minimum_tail_draws=None):
        ...
        from legacy.stocks.ml.model_selection import ScreenRouteDiagnostic
        scores = tuple((n, 0.0) for n, _ in cache.source_group_columns)
        attr = FeatureAttributionEvidence(family=family, fold_id=int(cache.fold.segment_id), source_group_scores=scores, selected_source_groups=tuple(n for n, _ in scores[:1]), schema_fingerprint=cache.schema.fingerprint)
...
def test_growth_admission_requires_positive_absolute_and_tail_bounds():
    from legacy.stocks.ml.contracts import ScreenEconomicEvidence, FamilyScreenEvidence, FeatureAttributionEvidence, ModelFamily
    from legacy.stocks.ml.model_selection import _screen_growth_admission_key
...
def test_resolve_screen_calendar_capacity_uses_rebalance_calendar():
    import polars as pl
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.model_selection import resolve_screen_calendar_capacity, deterministic_screen_sample_rows
...
def test_study_returns_structured_sample_capacity_failure_before_fit(monkeypatch):
    import tempfile, pathlib, polars as pl, numpy as np
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import ExecutionFrontierSettings, ModelSelectionComputeBudget, ModelSelectionStudySettings, NetAlphaTrainingRequest
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
    ...
    monkeypatch.setattr("legacy.stocks.ml.model_selection.prepare_screening_fold_cache", fake_cache)
    with tempfile.TemporaryDirectory() as tmp:
        registry = ModelArtifactRegistry(pathlib.Path(tmp))
        result = evaluate_model_selection_study(data, request, settings, registry=registry)
        assert result["status"] == "RESEARCH_ONLY"
...
def test_mlcmp_preflight_rejects_duplicate_or_route_missing_input_before_fit(monkeypatch):
    import tempfile, pathlib, polars as pl, numpy as np
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import NetAlphaResearchData, NetAlphaTrainingRequest, ExecutionFrontierSettings, ModelSelectionStudySettings, ModelSelectionComputeBudget, ModelFamily, FeatureAttributionEvidence
    from legacy.stocks.ml.model_selection import evaluate_model_selection_study, preflight_model_selection_inputs, resolve_reference_execution_cell
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.datasets import DatasetManifest
    from src.core.instruments import AssetKind
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles
    ...
    from legacy.stocks.ml.training import _build_label_join
    ...
    monkeypatch.setattr("legacy.stocks.research.folds.PurgedWalkForward._validate_no_duplicate_sessions", lambda self, samples: None)
...
def test_mlcmp_prepared_cache_aligns_sampled_keys_once_and_rejects_nonfinite(monkeypatch):
    import polars as pl, numpy as np
    from datetime import datetime, UTC, timedelta
    from legacy.stocks.ml.contracts import NetAlphaTrainingRequest, ModelSelectionStudySettings, ModelSelectionComputeBudget, ExecutionFrontierSettings
    from legacy.stocks.ml.model_selection import prepare_screening_fold_cache
    from legacy.stocks.ml.features import stock_net_alpha_v1_roles
    from legacy.stocks.research.folds import PurgedWalkForward
    from legacy.stocks.ml.training import _index_sessions, _locked_holdout, _build_label_join
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
    _roles = stock_net_alpha_v1_roles()
    rng = np.random.default_rng(1)
    ...
    from legacy.stocks.ml.model_selection import _build_prepared_screen_sample
    ...
    def spy(*a, **kw):
        called["n"]+=1
        return orig(*a, **kw)
    monkeypatch.setattr("legacy.stocks.ml.model_selection.fit_family_model", spy)
    # call screen_model_family with cache that has diagnostic should early return without fit
    from legacy.stocks.ml.contracts import ModelFamily
    from legacy.stocks.ml.model_selection import screen_model_family
...
def test_mlcmp_screen_uses_inner_selection_and_one_fit_per_family_fold(monkeypatch):
    ...
    def spy_fit(*a, **kw):
        ...
        return orig_fit(*a, **kw)
    monkeypatch.setattr("legacy.stocks.ml.family_specs.fit_family_model", spy_fit)
    monkeypatch.setattr("legacy.stocks.ml.model_selection.fit_family_model", spy_fit)
    ...
    from legacy.stocks.ml.model_selection import _select_inner_feature_groups
    from legacy.stocks.ml.contracts import ScreenSamplingPlan
...
def test_mlcmp_pools_bootstrap_once_per_family_without_cross_fold_concatenation(monkeypatch):
    ...
    import legacy.stocks.research.bootstrap as boot
    import legacy.stocks.ml.model_selection as msel
    calls=[]
    ...
    def spy_pooled(segments, block_length, n_bootstrap, seed):
        ...
        return orig(segments, block_length, n_bootstrap, seed)
    monkeypatch.setattr("legacy.stocks.research.bootstrap.pooled_segment_bootstrap_means", spy_pooled)
    monkeypatch.setattr("legacy.stocks.ml.model_selection.pooled_segment_bootstrap_means", spy_pooled)
...
def test_model_selection_unknown_screen_fault_propagates(monkeypatch) -> None:
    ...
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionStudySettings, ModelSelectionComputeBudget, NetAlphaTrainingRequest, NetAlphaResearchData, FeatureAttributionEvidence
    from legacy.stocks.ml.model_selection import ScreeningFoldCache, evaluate_model_selection_study
    from legacy.stocks.research.artifacts import ModelArtifactRegistry
    from src.core.costs import default_base_schedule, default_stress_schedule
    from tests.fixtures.stocks.helpers import stock_liquidity_model
    from src.core.datasets import DatasetManifest
    ...
    try:
        import legacy.stocks.ml.model_selection_preflight as pre
...
def test_fold_learning_panel_drops_unlabeled_rows_without_poisoning_fold() -> None:
    ...
    import polars as pl
    from legacy.stocks.research.folds import Fold
    from legacy.stocks.ml.model_selection import build_fold_learning_panel
...
def test_labeled_screen_sampler_respects_budget_without_four_k_capacity_gate() -> None:
    ...
    import polars as pl
    from legacy.stocks.ml.model_selection import sample_labeled_screen_rows
...
def test_ml_shortlist_keeps_valid_negative_economic_screen_for_oof() -> None:
    from legacy.stocks.ml.contracts import FeatureAttributionEvidence, FamilyScreenEvidence, ModelFamily, ScreenMlEvidence
    from legacy.stocks.ml.model_selection import select_ml_screen_shortlist
...
def test_settings_resolve_reference_cell_from_wide_frontier() -> None:
    ...
    from legacy.stocks.ml.contracts import ExecutionFrontierSettings, NetAlphaTrainingRequest
    from legacy.stocks.ml.model_selection import build_model_selection_study_settings, resolve_model_selection_reference_cell
...
def test_screen_route_rejects_equal_top_k_cross_section() -> None:
    ...
    from legacy.stocks.ml.model_selection import _screen_route_utility_series
...
def test_model_selection_facade_reexports_real_study_owner() -> None:
    from legacy.stocks.ml import model_selection
    from legacy.stocks.ml.model_selection_study import evaluate_model_selection_study

    assert model_selection.evaluate_model_selection_study is evaluate_model_selection_study
    assert evaluate_model_selection_study.__module__ == 'legacy.stocks.ml.model_selection_study'
...
def test_model_selection_panel_rejects_duplicate_feature_keys() -> None:
    ...
    from legacy.stocks.ml.model_selection_panel import build_fold_learning_panel
    from legacy.stocks.research.folds import Fold
...
def test_model_selection_facade_monkeypatch_seam_reaches_canonical_study(monkeypatch) -> None:
    from legacy.stocks.ml import model_selection
    from legacy.stocks.ml import model_selection_study
