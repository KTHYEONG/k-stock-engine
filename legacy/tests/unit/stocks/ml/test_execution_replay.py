...
from legacy.stocks.ml.replay_preparation import (
...
def test_prepared_replay_allocation_02_uses_segment_allocator(
    ...
    from legacy.stocks.trading.portfolio_constructor import PreparedAllocationMarket
    from legacy.stocks.workflows import trading_cycle
...
def test_interval_evidence_growth_recovery() -> None:
    ...
    from legacy.stocks.ml.execution_replay import _decision_interval_log_growth
...
def test_replay_batch_01_parity_and_build_bound(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from legacy.stocks.backtesting.engine import PreparedReplayMarket
...
def test_replay_batch_02_fail_closed_incompatible_input() -> None:
    ...
    from legacy.stocks.ml.execution_replay import _validate_batch_request_compatibility
...
def test_replay_budget_01_breach_fails_closed_before_any_build(
    ...
    from legacy.stocks.ml.replay_resources import MemoryBudgetExceededError
...
def test_replay_cgroup_01_effective_limit_is_min_of_finite_contributors() -> None:
    ...
    """
    from legacy.stocks.ml.replay_resources import resolve_effective_memory_limit
...
def test_SCENARIO_SMALL_ACCOUNT_CAGR_04_INTEGER_REPLAY_BASIS():
    ...
    from legacy.stocks.ml.contracts import NetAlphaTrainingRequest, PortfolioSettings
    ...
    from legacy.stocks.ml.contracts import AccountCertificationSettings
...
def test_SCENARIO_SMALL_ACCOUNT_LOT_05_CAUSAL_BUFFER():
    ...
    import polars as pl
    from datetime import UTC, datetime, timedelta
    from legacy.stocks.trading.portfolio_constructor import derive_causal_open_gap_buffer_bps
    import pytest
    base = datetime(2024,1,1,tzinfo=UTC)
    rows=[]
    for i in range(5):
...
def test_MLCMP_REPLAY_DIAG_03():  # noqa: N802
    ...
    from legacy.stocks.ml.replay_aggregation import CandidateReplayAccumulator, SegmentExecutionSummary
...
def test_stock_only_profile_uses_existing_tax_bound_lot_replay() -> None:
    from legacy.stocks.config.research import policy_profiles_with_stock_only_small_capital
    from legacy.stocks.ml.contracts import AccountCertificationSettings
    from legacy.stocks.ml.training import _risk_policy_for_profile

    profile = policy_profiles_with_stock_only_small_capital()[-1]
    request = __import__('legacy.stocks.ml.contracts', fromlist=['NetAlphaTrainingRequest']).NetAlphaTrainingRequest(
