...
def test_model_candidate_binds_k_only_for_exact_k_ranker():
    from legacy.stocks.ml.contracts import FeatureAttributionEvidence, ModelFamily, ModelSelectionCandidate
    ...
    from legacy.stocks.ml.family_specs import family_spec, fit_family_model
    import numpy as np
    import polars as pl
...
def test_nested_prefix_selection_never_reads_outer_validation():
    from legacy.stocks.ml.contracts import ModelFamily
    from legacy.stocks.ml.features import fit_research_feature_schema, stock_net_alpha_v1_roles
    from legacy.stocks.ml.family_specs import family_spec, fit_family_model, family_feature_columns
    import polars as pl
    ...
    # Build matrices
    from legacy.stocks.ml.features import apply_research_feature_schema
...
def test_segmented_block_bootstrap_pools_without_crossing_segments():
    from legacy.stocks.ml.model_selection import segmented_moving_block_lower_bound
    import numpy as np
...
def test_negative_finite_screen_still_reaches_bounded_exact_replay(monkeypatch):
    from legacy.stocks.ml.contracts import ModelFamily, ModelSelectionStudySettings, ModelSelectionComputeBudget, NetAlphaTrainingRequest, NetAlphaResearchData
    ...
    from legacy.stocks.ml.contracts import FeatureAttributionEvidence, FamilyScreenEvidence, ScreenEconomicEvidence
...
def test_champion_is_stress_argmax_with_deterministic_tiebreaks():
    from legacy.stocks.ml.contracts import FeatureAttributionEvidence, ModelFamily, ModelSelectionCandidate
    from legacy.stocks.ml.model_selection import ReplayCandidateEvidence, select_model_selection_champion
...
def test_reference_execution_cell_resolves_by_value_not_position():
    from legacy.stocks.ml.contracts import ExecutionFrontierSettings, NetAlphaTrainingRequest, ModelSelectionStudySettings, PortfolioSettings
    from legacy.stocks.ml.model_selection import resolve_reference_execution_cell
...
def test_linear_rank_interaction_is_hidden_from_tree_families():
    from legacy.stocks.ml.contracts import ModelFamily
    from legacy.stocks.ml.features import fit_research_feature_schema, stock_net_alpha_v1_roles
    from legacy.stocks.ml.family_specs import family_spec, family_feature_columns
    import polars as pl
    from datetime import datetime, UTC, timedelta
    import numpy as np
...
def test_ML_SCREEN_02_deterministic_inner_selection() -> None:
    ...
    from datetime import datetime, UTC, timedelta
    import numpy as np
    from legacy.stocks.ml.contracts import ModelFamily, ScreenSamplingPlan, NetAlphaTrainingRequest
